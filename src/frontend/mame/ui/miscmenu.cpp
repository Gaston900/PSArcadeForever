...
menu_autofire::menu_autofire(mame_ui_manager &mui, render_container &container) : menu(mui, container)
{
}
...
void menu_autofire::handle(event const *ev)
{
	bool changed = false;
...
//	item_append("Press 1-9 to Config", NULL, 0, NULL);
//	item_append(MENU_SEPARATOR_ITEM, NULL, 0, NULL);
	...
	for (auto &port : machine().ioport().ports())
	{
		for (auto &field : port.second->fields())
		{
			...
			if (name.c_str() != nullptr && type >= IPT_CUSTOM1 && type < IPT_CUSTOM1 + MAX_CUSTOM_BUTTONS)
			{
